...
const finalizeCasePills = (
  ...
  if (!meetsPolicy) {
    if (caseNumber !== 1 && fallbackCase1Pills.length > 0) {
      console.warn(
        `[Recommendations POST] ${caseName} does not meet policy (business=${hasBusiness}, anticipate=${anticipateCount}, entice=${enticeCount}); falling back to case1`
      )
      return fallbackCase1Pills.slice(0, CASE_POLICY.maxPills)
    }
    console.warn(
      `[Recommendations POST] ${caseName} cannot meet policy and no fallback is available; keeping best validated pills`
    )
  }
